...
│   │   ├── chat/            # Chat interface page (Phase 3)
│   │   ├── layout.tsx       # Root layout
│   │   └── page.tsx         # Home page
│   ├── components/
│   │   ├── ui/              # Reusable UI components
│   │   └── chat/            # Chat specific components (ChatInterface, MessageList)
│   ├── lib/                 # Utility functions
│   │   ├── auth.ts          # Better Auth client config
│   │   └── env.ts           # Environment validation
│   └── services/
│       └── api.ts           # Backend API client (includes chat.sendMessage)
...
- Place feature-specific components (like Chat) in `src/components/<feature>/`
- Use Tailwind for styling, avoid inline styles
- **Chat Interface**: Located in `src/components/chat/ChatInterface.tsx`. Handles message history and streaming interaction.
